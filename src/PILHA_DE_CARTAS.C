...
PIL_tpCondRet PIL_Verifica ( PIL_tppPilha pPilha , int *numErros )
{
	...
	if( !LIS_VerificaCorrenteNull( pPilha->pListaCartas ))
	{
		...
		if( pPilha != NULL && pPilha->qtdNos == 0)
		{
			...
			//vai tirar : pilha->pListaCartas == NULL 
			ou 
			...
			//tiramos este teste de assertiva estrutural, porque a partir do momento que exite uma pilha, existe uma lista e o contador ficava sempre 0
			/*if (retLis == 5 )
			{
			CNT_CONTAR("ok-lista-nao-existe");	
			}
			else */
			if ( retLis == 1)
			{
